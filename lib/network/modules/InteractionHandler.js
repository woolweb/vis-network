...
class InteractionHandler {
  ...
  onMouseWheel(event) {
    ...
    if (event.deltaY !== 0) {
      ...
      let view = this.body.view;
      var scale = view.scale;
      let scaleSteps = view.scaleSteps;
      if (!scaleSteps) {
        scaleSteps = view.scaleSteps = [];
      }
      let tempScale = scale * (1 + (event.deltaY < 0 ? 1 : -1) * (this.options.zoomSpeed * 0.1));
      let isNeedUpdateScale = tempScale > 0.00001 && tempScale < 10
