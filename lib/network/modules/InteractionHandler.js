  ...
  onMouseWheel(event) {
    if (this.options.zoomView === true) {
      ...
      if (event.deltaY !== 0) {
        ...
        var view = this.body.view;
        var scale = view.scale;
        var scaleSteps = view.scaleSteps;
        if (!scaleSteps) {
          scaleSteps = view.scaleSteps = [];
        }
        var tempScale = scale * (1 + (event.deltaY < 0 ? 1 : -1) * (this.options.zoomSpeed * 0.1));
        var isNeedUpdateScale = tempScale > 0.00001 && tempScale < 10

        var realScale;
        if(isNeedUpdateScale){
          //每次缩放，会导致缩放因子发生变化，通过一个集合来保存生成的缩放因子历史记录，以便于尽量维持缩放因子的值
          if (event.deltaY > 0) {
            if (view.zoomIndex === undefined) {
              view.zoomIndex = 0;
            } else {
              view.zoomIndex--;
            }
            if (!scaleSteps[view.zoomIndex]) {
              if (view.zoomIndex < 0) {
                view.zoomIndex = 0;
              }
              scaleSteps.unshift(tempScale);
              realScale = tempScale;
            } else {
              realScale = scaleSteps[view.zoomIndex];
            }
          } else {
            if (view.zoomIndex === undefined) {
              view.zoomIndex = 0;
